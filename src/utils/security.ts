...
export const handleSecureError = (error: unknown): { message: string; code?: string } => {
  // Don't expose sensitive error details to client
  if ((error as Error).message?.includes('password') || (error as Error).message?.includes('token')) {
    return { message: 'Authentication error occurred', code: 'AUTH_ERROR' };
  }
  
  if ((error as Error).message?.includes('database') || (error as Error).message?.includes('sql')) {
